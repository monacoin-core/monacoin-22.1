#!/usr/bin/env python3
# Copyright (c) 2017-2020 The Monacoin Core developers
...
class MultiWalletTest(MonacoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            exp_stderr = r"Error: SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another monacoind?"
        else:
        ...
        if self.options.descriptors:
            assert_raises_rpc_error(-4, "Wallet file verification failed. SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another monacoind?", self.nodes[0].loadwallet, wallet_names[0])
        else:
