...
- #11962 `0a01843` add seed.monacoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in MONA (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI monacoin:// (krab)
...
- #13528 `b877c39` Move MonacoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libmonacoin-util to libmonacoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for monacoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling monacoin-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use MonacoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to monacoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use MonacoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use MonacoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use MonacoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for MonacoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_monacoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.monacoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_monacoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in monacoind systemd service file (Flowdalic)
- #12652 `55f490a` monacoin-cli: Provide a better error message when monacoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to monacoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for monacoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` monacoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix monacoin-cli --version (Empact)
...
- #12877 `2b54155` Use monacoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update monacoin-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust monacoincore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed monacoin-cli -help output for help2man (hebasto)
