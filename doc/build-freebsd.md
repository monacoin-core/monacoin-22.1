...
#### Wallet Dependencies
It is not necessary to build wallet functionality to run monacoind or the GUI. To enable legacy wallets, you must install `db5`. To enable [descriptor wallets](https://github.com/monacoin/monacoin/blob/master/doc/descriptors.md), `sqlite3` is required. Skip `db5` if you intend to *exclusively* use descriptor wallets
