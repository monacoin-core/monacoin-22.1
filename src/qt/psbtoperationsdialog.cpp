...
void PSBTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = MonacoinUnits::format(m_wallet_model->getOptionsModel()->getDisplayUnit(), out.nValue);
...
std::string PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(tr(" * Sends %1 to %2")
            .arg(MonacoinUnits::formatWithUnit(MonacoinUnits::MONA, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(MonacoinUnits::formatWithUnit(MonacoinUnits::MONA, *analysis.fee));
