...
bool parseMonacoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!MonacoinUnits::parse(MonacoinUnits::MONA, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatMonacoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MonacoinUnits::format(MonacoinUnits::MONA, info.amount, false, MonacoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", MONACOIN_CONF_FILENAME));
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("monacoin-%s.desktop", chain);
}
